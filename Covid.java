...
public enum Covid {
	int [] drate = new int [] {1,3,10}; //in promiles
	double TRANSMITION_RATE = 2.4;
	int [] TIME_TO_RECOVERY = new int [] {12,22}; //days
	int [] TIME_TO_CONTAGIOUS = new int [] {1,5}; //days
	
	
	
 public	double howContagious(int time_iterator, Person vector) {
	 double distance =1; //hard-coded for now
	int t = i; //timeline - all project iterator?
 ...
 public void is_Exposed(/*ArrayList*/Person vector) {
	 ...
	 final double AT_RISK = 15*3*1/java.lang.Math.pow(2, -2);  //3 steps in a minute //possible enum
